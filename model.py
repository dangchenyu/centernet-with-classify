...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
                 last_level, head_conv, batch_size,out_channel=0):
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                ...
                if 'cla' in head:
                    fc=nn.Sequential(BasicBlock_nodil(channels[self.first_level],channels[self.first_level]),nn.Conv2d(channels[self.first_level], head_conv,
    ...
    def forward(self, x):
        ...
        z = {}
        for head in self.heads:
            if y[-1].shape[0] != self.batch_size:
                self.batch_size=y[-1].shape[0]
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dla{}'.format(num_layers), heads,
                   ...
                   final_kernel=1,
                   last_level=5,
                   head_conv=head_conv,batch_size=1)
    return model
...
if __name__ == '__main__':
    ...
    model = DLASeg('dla{}'.format(34), {'hm':80,'wh':2,'reg':2,'cla':1},
                   ...
                   final_kernel=1,
                   last_level=5,
                   head_conv=256,batch_size=1).cuda()
